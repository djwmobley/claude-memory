...
CREATE TABLE IF NOT EXISTS assertions (
  ...
  -- suppression_kind values: 'superseded' | 'downvoted_terminal' | 'downvoted_probation' | 'retired' | 'reality_reconciled' | 'stale_pointer'
  ...
  --   'stale_pointer' added by cm#297 close-time bulk pass (_suppressStaleLegacyPointers):
  --   every extracted pointer classified IN_REPO_STALE and the predicate is not an intent
  --   predicate; always paired with invalid_at in the same UPDATE.
  valid_at         TEXT,
  invalid_at       TEXT,
  suppression_kind TEXT    CHECK (suppression_kind IN ('superseded', 'downvoted_terminal', 'downvoted_probation', 'retired', 'reality_reconciled', 'stale_pointer')),
