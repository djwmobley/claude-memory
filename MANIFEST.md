...
| `scripts/test-doc-lint.js` | CI gate enforcing glossary See-also cross-reference resolution, doc-to-real-command validity (/handoff: slash commands + handoff.js engine subcommands), suppression_kind enum sync across SQL schema, cmdPrune validKinds, and the glossary Prune bullet, and MANIFEST.md index integrity (every indexed path exists on disk and is not gitignored) | updated (D4 added: MANIFEST index-integrity check — path existence + not-gitignored, via `git check-ignore`) |
