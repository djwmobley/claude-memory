...
| `scripts/test-doc-lint.js` | CI gate enforcing glossary See-also cross-reference resolution, doc-to-real-command validity (/handoff: slash commands + handoff.js engine subcommands), suppression_kind enum sync across SQL schema, cmdPrune validKinds, and the glossary Prune bullet, and MANIFEST.md index integrity (every indexed path exists on disk, is not gitignored, and is not a malformed unwrapped-path entry) | updated (D4c added: a MANIFEST table row whose first cell looks path-like but isn't backtick-wrapped now fails the gate instead of being silently skipped) |
