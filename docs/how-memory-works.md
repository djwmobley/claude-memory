...
**Session intent rows.** As of the north-star inversion, the three most session-critical items — the TL;DR, open threads, and quick references — are persisted as queryable Postgres assertion rows, not just as prose in a file. Three dedicated predicates handle this: `session_tldr` (one live row per project, latest supersedes prior), `open_thread` (one live row per thread-key), and `quick_reference` (one live row per project). These rows are written at `/handoff:close` through the same gated write path (`writeAssertionWithSupersession`) that all other assertions use, so the consolidation gate applies — a cross-session restatement alone does not forge a `consolidated` tier. Any error during intent persistence is caught per-row and logged; the close operation still succeeds. On resume, the loader surfaces these rows in a `### Session intent` section, ordered by decay-adjusted confidence.

**The handoff file (`handoff.md`).** A thin pointer lives at `~/.claude/projects/<your-project>/handoff.md`. After the north-star inversion, this file no longer holds prose TL;DR, open threads, or quick references — those live in Postgres (see above). The file carries only the project metadata header (project ID, last close timestamp, contract name, session summary counts) and refers the reader to Postgres for session-driving content. You do not edit this file by hand. `/handoff:close` renders it; the loader reads the contract name from it at session start.
...
**The three north-star tenets:**

1. **Lossless fidelity across the seam.** No prior plan or decision is lost when you cross a session boundary. The baton is complete.
2. **Lean, decay-ranked default resume.** The loader surfaces the minimum context needed to resume effectively — ordered by decay-adjusted confidence, not alphabetically or chronologically. Bootstrap token spend is minimized.
3. **Resurrection on demand.** A topic that has decayed to the back shelf can be pulled forward explicitly via a query. Decay is a volume knob, not a delete button.

**The load-bearing premise:** continuity must live as queryable Postgres rows, not as markdown prose. `handoff.md` is a thin pointer; the assertions table is the source of truth.
