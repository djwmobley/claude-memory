...
**Session intent persistence (north-star inversion).** At close time, the TL;DR, open threads, and quick references from the payload are persisted as queryable Postgres assertion rows using three 1:1 predicates: `session_tldr`, `open_thread`, and `quick_reference`. These rows are written through the same gated write path (`writeAssertionWithSupersession`) used by all other assertions — the L0/L2 consolidation gate applies; a cross-session restatement alone does not forge a `consolidated` tier. Any error during intent persistence is caught per-row and logged; the close operation still succeeds — but it is never silent: a failure to persist any of the three prints a `DIVERGENCE: <predicate> NOT PERSISTED — <error>` line in the close/checkpoint summary AND appears in the rendered `handoff.md`'s `## Degraded` section, so a dropped TL;DR is always operator-visible. This is purely a visibility change — the exit code is unaffected even under `close_degraded_exit_mode=strict`. The `handoff.md` file is rendered as a **thin pointer** (metadata header only; session content lives in Postgres), not a prose narrative of the session.
